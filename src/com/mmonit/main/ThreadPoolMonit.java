...
public class ThreadPoolMonit {
	...
	/**
	 * 使用固定线程数的newFixedThreadPool管理线程池 共享资源：ServerSocket ConcurrentHashMap
	 * */
	...
	static {
		monitserverport = Integer.parseInt((PropertiesUtil.getProValue("monitconf.properties", "monitserverport")!=null)?PropertiesUtil.getProValue("monitconf.properties", "monitserverport"):"8080");
		isactivemq = Boolean.parseBoolean((PropertiesUtil.getProValue("monitconf.properties", "isactivemq")!=null)?PropertiesUtil.getProValue("monitconf.properties", "isactivemq"):"false");
		threadpoolsize = Integer.parseInt((PropertiesUtil.getProValue("monitconf.properties", "threadpoolsize")!=null)?PropertiesUtil.getProValue("monitconf.properties", "threadpoolsize"):"4");
	}
		...
		case 1 :
			while (true) {
				...
				try {
					...
					executorService.execute(new MainHandlerNOMQ(socket,
							concurrentHashMap, normalMQueue));
					...
					executorService.execute(new DetectMonitMapHandler(socket,
							concurrentHashMap));
		...
		case 2 :
			while (true) {
				...
				try {
					socket = serverSocket.accept();
					/* 将得到的socket投入线程池中 重新定义concurrentHashMap */
					executorService.execute(new MainHandler(socket,
							concurrentHashMap, normalMQueue, eventMQueue));
					/* 得到消息队列，使用工作线程对其进行数据IO操作，减轻socket的压力 */
					executorService.execute(new WorkHandler(normalMQueue));
					/* activeMQ 的处理消息队列 */
	...
	private void init() {
		/* 启动容器 */
		SpringUtil.start();
		MonitSerBusOperator monitSerBusOperator = (MonitSerBusOperator) SpringUtil
